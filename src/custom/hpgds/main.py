...
from custom.hpgds.reports.report_builder import build_hpgds_reports
from worklog_analytics.reports.console_renderer import render_reports
from worklog_analytics.reports.workbook_renderer import write_workbook
...
def main():
    ...
    studio_groups_config = load_json(
        Path("src/custom/hpgds/configs/studio_groups.json"), True
    )
    ...
    core_reports = run_reports(context)
    hpgds_reports = build_hpgds_reports(
        context.worklogs,
    ...
    all_reports = []
    all_reports.extend(core_reports)
    all_reports.extend(hpgds_reports)

    run_debug_reports(context)
    run_review_items_report(context)

    render_reports(all_reports)
    write_workbook(reports=all_reports, template_file=template_file, output_file=output_file)
